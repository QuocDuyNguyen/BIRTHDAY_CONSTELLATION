...
export function ZodiacScene({ phase, sign, dimmed = false }: ZodiacSceneProps) {
  ...
  return <motion.div className={`zodiac-scene ${dimmed ? "scene-in-background" : ""}`} style={{ "--zodiac-primary": sign.primaryColor, "--zodiac-secondary": sign.secondaryColor } as React.CSSProperties} initial={{ opacity: 0 }} animate={{ opacity: dimmed ? .16 : active ? 1 : 0 }} transition={{ duration: 1.05, ease: [0.16, 1, 0.3, 1] }} aria-label={`Cung hoàng đạo ${sign.name}`}>
    <motion.div className="zodiac-ring" initial={{ opacity: 0, scale: .9 }} animate={{ rotate: 360, opacity: active ? 1 : 0, scale: active ? 1 : .9 }} transition={{ rotate: { duration: 60, repeat: Infinity, ease: "linear" }, opacity: { duration: 1.05, delay: .12, ease: "easeOut" }, scale: { duration: 1.2, delay: .12, ease: [0.16, 1, 0.3, 1] } }}>{zodiacSymbols.map((symbol, index) => <span key={symbol} className={symbol === sign.symbol ? "active" : ""} style={{ transform: `rotate(${index * 30}deg) translateY(-min(31vw, 225px)) rotate(-${index * 30}deg)` }}>{symbol}</span>)}</motion.div>
