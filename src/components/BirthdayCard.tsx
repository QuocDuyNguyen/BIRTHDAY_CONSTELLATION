...
export function BirthdayCard({ data, sign, age, visible, anchorToEnvelope, onReplay, onClose, onBlow, blown, musicOn, onMusicToggle, typingKey }: BirthdayCardProps) {
  ...
  useEffect(() => { setText(""); setDone(false); if (!visible) return; let index = 0; const timer = window.setInterval(() => { index += 1; setText(fullText.slice(0, index)); if (index >= fullText.length) { window.clearInterval(timer); setDone(true); } }, 36); return () => window.clearInterval(timer); }, [fullText, typingKey, visible]);
  return <motion.section className={`card-wrap ${anchorToEnvelope ? "letter-card-position" : ""}`} initial={false} animate={{ opacity: visible ? 1 : 0, y: visible ? 0 : 70, scale: visible ? 1 : 0.78, pointerEvents: visible ? "auto" : "none" }} transition={{ type: "spring", stiffness: 120, damping: 18 }}>
