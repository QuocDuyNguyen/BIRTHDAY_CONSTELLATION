...
export function BirthdayCard({ data, sign, age, visible, anchorToEnvelope, onReplay, onClose, onBlow, blown, musicOn, onMusicToggle, typingKey }: BirthdayCardProps) {
  ...
  return <motion.section className={`card-wrap ${anchorToEnvelope ? "letter-card-position" : ""}`} initial={false} animate={{ opacity: visible ? 1 : 0, y: visible ? 0 : 70, scale: visible ? 1 : 0.78, pointerEvents: visible ? "auto" : "none" }} transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}>
