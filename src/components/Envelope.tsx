...
export function Envelope({ phase, onOpen }: EnvelopeProps) {
  ...
  const flight = phase === "envelope" && !returning ? { opacity: 1, x: isMobile ? [0, 86, -78, 35, finalX] : [0, 180, -210, 145, finalX], y: isMobile ? [0, -130, -72, 65, finalY] : [0, -135, 125, -82, finalY], rotate: isMobile ? [0, 18, -17, 13, 0] : [0, 22, -23, 18, 0] } : { opacity: show ? (phase === "card-message" || phase === "complete" ? 0 : 1) : 0, x: open ? finalX : 0, y: open ? finalY : show ? 0 : 80, rotate: 0, scale: show ? 1 : 0.7 };
  return <motion.div className="envelope-wrap" initial={false} animate={flight} transition={phase === "envelope" && !returning ? { duration: 4.6, ease: "easeInOut" } : { duration: 0.38, ease: [0.22, 1, 0.36, 1] }} onAnimationComplete={() => { if (phase === "envelope") setArrived(true); }}><div className={`envelope-cosmo ${open ? "is-opening" : ""}`} aria-hidden="true"><i /><i /><i /><i /><i /></div><button className={`envelope ${open ? "is-open" : ""} ${arrived ? "is-arrived" : ""}`} onClick={onOpen} disabled={open || phase !== "envelope" || !arrived} aria-label="Mở phong bì sinh nhật"><div className="envelope-paper">♡</div><div className="envelope-front" /><div className="envelope-flap" />{phase === "envelope" && arrived && <span className="envelope-hint">Chạm để mở thư</span>}</button></motion.div>;
}
